"""v31: condition-balanced training.

OOF diagnosis showed v20 MAE_v=10 at T=160 bio=0 (115 train samples) but
MAE_v=103 at T=150 (28 samples split across bio/time regimes).  Condition
imbalance: (T=160, t=168, bio=0) = 115/167 = 69%; rare regimes have 1-14.

We reweight each train sample to 1/sqrt(condition_frequency) so rare regimes
dominate enough gradient to be learned.  Test set has the same condition
distribution as train, so this doesn't create a domain-shift issue.

Also uses a larger epoch budget (220) and no pseudo-labels, to keep the probe
clean.
"""
...
def assign_balance_weights(samples, mix_train, exponent=0.5):
    """Set sample.weight <- 1 / freq(condition_bucket)^exponent, renormalised.

    exponent 0.0 = no balancing, 1.0 = full balancing, 0.5 = sqrt-balancing
    (keeps common regimes dominant but lifts rare ones noticeably).
    """
    cond_of = {}
    for sid, grp in mix_train.groupby("scenario_id"):
        r = grp.iloc[0]
        cond_of[sid] = (float(r[COL_TEMP]), float(r[COL_TIME]),
                        float(r[COL_BIO]))
    buckets = [cond_of[s.scenario_id] for s in samples]
    counts = Counter(buckets)
    n = len(samples)
    for s, b in zip(samples, buckets):
        f = counts[b] / n  # relative frequency
        s.weight = float(1.0 / (f ** exponent))
    # Renormalise so mean weight == 1.0 (keeps overall loss scale similar).
    mean_w = float(np.mean([s.weight for s in samples]))
    for s in samples:
        s.weight /= mean_w
...
def main():
    ...
    ap.add_argument("--out_dir", default="artifacts_v31")
    ap.add_argument("--n_folds", type=int, default=5)
    ap.add_argument("--epochs", type=int, default=220)
    ap.add_argument("--seeds", type=int, default=3)
    ap.add_argument("--balance_exp", type=float, default=0.5,
                    help="Condition balance exponent (0=off, 0.5=sqrt, 1=full).")
    ...
    assign_balance_weights(train_samples, mix_train, exponent=args.balance_exp)
    w_stats = [s.weight for s in train_samples]
    print(f"Sample weight stats: min={min(w_stats):.3f} "
          f"max={max(w_stats):.3f} mean={np.mean(w_stats):.3f}")
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(kf.split(scenarios)):
        ...
        for seed in range(args.seeds):
            print(f"\n[v31 fold {fold_idx+1}/{args.n_folds} seed {seed}]")
    ...
    print(f"\n=== v31 OOF: visc {mv:.3f}, ox {mo:.3f}, norm {mv/std_v/2 + mo/std_o/2:.4f} ===")

    # Per-condition OOF breakdown
    cond_of = {}
    for sid, grp in mix_train.groupby("scenario_id"):
        r = grp.iloc[0]
        cond_of[sid] = (float(r[COL_TEMP]), float(r[COL_TIME]), float(r[COL_BIO]))
    conds = np.array([cond_of[s] for s in scenarios])
    print("\n=== Per-condition OOF MAE ===")
    for c in sorted(set(map(tuple, conds))):
        m = np.all(conds == np.array(c), axis=1)
        if m.sum() < 2: continue
        mv_c = np.mean(np.abs(err[m, 0]))
        mo_c = np.mean(np.abs(err[m, 1]))
        print(f"  T={c[0]:.0f} t={c[1]:.0f} bio={c[2]:.0f}: n={m.sum()} "
              f"MAE_v={mv_c:.1f} MAE_o={mo_c:.1f}")
